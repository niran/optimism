...
func InteropGenInteropSystem(dest *DefaultInteropSystemIDs, interopOffset uint64) stack.Option {
	...
	opt.Add(func(o stack.Orchestrator) {
		paths, err := contractPaths()
		o.P().Require().NoError(err)

		WithInteropGen(
			stack.L1NetworkID(ids.L1.ChainID()),
			ids.Superchain,
			ids.Cluster,
			[]stack.L2NetworkID{ids.L2A, ids.L2B},
			paths,
			interopOffset,
		)(o)
	})
