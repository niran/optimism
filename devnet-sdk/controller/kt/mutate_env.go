...
type metaService []*descriptors.Service

func (s metaService) getEndpoints() descriptors.EndpointMap {
	if len(s) == 0 {
		return nil
	}
...
func (s metaService) setEndpoints(endpoints descriptors.EndpointMap) {
	for _, svc := range s {
		svc.Endpoints = endpoints
	}
}

func (s metaService) refreshEndpoints(serviceCtx interfaces.ServiceContext) {
...
func findSvcInEnv(env *descriptors.DevnetEnvironment, serviceName string) metaService {
	if svc := findSvcInChain(env.L1, serviceName); svc != nil {
		return metaService{svc}
	}

	var services metaService = nil
