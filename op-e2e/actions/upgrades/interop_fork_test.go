...
func TestInteropUpgrade(gt *testing.T) {
	...
	activationBlockNumber := activationBlockHeaderA.Number
	...
	VerifyContractsDeployedCorrectly(t, actors, activationBlockTxs, activationBlockNumber)

	actors.ActBatchAllAndMine(t)
	...
	require.Equal(t, activationBlockNumber.Uint64(), statusA.UnsafeL2.ID().Number)
	require.Equal(t, activationBlockNumber.Uint64(), statusA.CrossUnsafeL2.ID().Number)
	require.Equal(t, activationBlockNumber.Uint64(), statusA.LocalSafeL2.ID().Number)
	require.Equal(t, activationBlockNumber.Uint64(), statusA.SafeL2.Number)
	require.Equal(t, uint64(0), statusA.FinalizedL2.Number)

	statusB := actors.ChainB.Sequencer.SyncStatus()
	require.Equal(t, activationBlockNumber.Uint64(), statusB.UnsafeL2.ID().Number)
	require.Equal(t, activationBlockNumber.Uint64(), statusB.CrossUnsafeL2.ID().Number)
	require.Equal(t, activationBlockNumber.Uint64(), statusB.LocalSafeL2.ID().Number)
	require.Equal(t, activationBlockNumber.Uint64(), statusB.SafeL2.Number)
	require.Equal(t, uint64(0), statusB.FinalizedL2.Number)
}

func VerifyContractsDeployedCorrectly(t helpers.Testing, actors *dsl.InteropActors, activationBlockTxs []*types.Transaction, activationBlockNumber *big.Int) {
	...
	utils.RequireUpgradeSuccessful(t, actors, derive.CrossL2InboxAddress, predeploys.CrossL2InboxAddr, activationBlockNumber)
	utils.RequireUpgradeSuccessful(t, actors, derive.L2ToL2MessengerAddress, predeploys.L2toL2CrossDomainMessengerAddr, activationBlockNumber)
}
